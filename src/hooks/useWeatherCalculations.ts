...
export function useWeatherCalculations(weatherData: ExtendedWeatherData | null, unit: WeatherUnit, now: Date) {
  ...
  // OPTIMITZACIÓ: Clau estable per hora
  ...
  const currentRainProbability = useMemo(() => {
     ...
     const hourIdx = weatherData.hourly.time.findIndex((t: string) => {
  ...
  const currentFreezingLevel = useMemo(() => {
      ...
      const nowMs = shiftedNow.getTime();
      const currentIdx = weatherData.hourly.time.findIndex((t: string) => {
          const tMs = new Date(t).getTime();
          return tMs <= nowMs && (tMs + 3600000) > nowMs;
      });
  ...
  const currentBg = useMemo(() => {
    ...
    const getDynamicBackground = (code: number, isDay = 1) => {
  ...
  const chartData = useMemo(() => {
    ...
    const idx = weatherData.hourly.time.findIndex((t: string) => new Date(t).getTime() >= nowTime);
    let startIndex = 0;
    ...
    const getSafeVal = (key: string, i: number, def: any = 0) => {
  ...
  const comparisonData = useMemo(() => {
      ...
      const sliceModel = (modelData: any[]) => {
         ...
         return modelData.slice(startIndex, endIndex).map((d: any, i: number) => {
             if (!d) return null;
             return {
