...
import { Clock, Calendar, TrendingUp, ChevronRight, Droplets, Snowflake } from 'lucide-react';
...
interface ForecastSectionProps {
    ...
    showCharts?: boolean; // NOVA PROPIETAT OPCIONAL
}
  ...
  return (
    <div className="flex flex-col gap-6 w-full">
        ...
        <div className="bento-card p-6">
            <h3 className="label-upper mb-4 flex items-center gap-2"><Clock className="w-4 h-4 text-indigo-400"/> {t.hourlyEvolution} (24h)</h3>
                ...
                {chartData.map((h, i) => {
                    ...
                    return (
                        <div key={i} className="flex flex-col items-center min-w-[4.5rem] snap-start p-3 rounded-2xl bg-white/5 border border-white/5 hover:bg-white/10 transition-colors">
                            ...
                            <div className="mb-2 scale-110 drop-shadow-md">{getWeatherIcon(h.code, "w-8 h-8", h.isDay)}</div>   
                            <span className="text-lg font-bold text-white mb-1">{Math.round(h.temp)}°</span>
                            
                            {precipProb > 0 ? (
                                <div className="h-1 w-full bg-slate-700/50 rounded-full overflow-hidden mt-1">
                                    <div className={`h-full ${isSnow ? 'bg-slate-200' : 'bg-blue-500'}`} style={{ width: `${Math.min(precipProb, 100)}%` }}></div>
                                </div>
        ...
        {/* 2. DAILY FORECAST */}
        <div className="bento-card p-6">
            <h3 className="label-upper mb-4 flex items-center gap-2"><Calendar className="w-4 h-4 text-amber-400"/> {t.forecast7days}</h3>
                ...
                {dailyData.time.slice(1).map((day: string, idx: number) => {
                    ...
                    return (
                        <button key={day} onClick={() => onDayClick(i)} className="flex items-center justify-between p-3.5 bg-white/5 hover:bg-white/10 border border-white/5 rounded-2xl transition-all group w-full active:scale-[0.99]">
                            ...
                            <div className="flex items-center gap-2 w-auto justify-end min-w-[4rem]">
                                {precipSum > 0 && (
                                    <div className={`flex items-center gap-1 text-[10px] font-bold px-2 py-1 rounded-lg border ${isSnow ? 'text-white bg-slate-500/20 border-white/20' : 'text-blue-300 bg-blue-500/10 border-blue-500/20'}`}>
        ...
        {/* 3. CHARTS (Condicional amb la nova prop) */}
