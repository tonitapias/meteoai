...
export const WeatherStatsGrid = ({ windSpeed, windGusts, windDirection, humidity, apparentTemp }: WeatherStatsGridProps) => {
  // DOCTRINA RISC ZERO: Escut visual per a valors nuls
  const formatValue = (val: number | null | undefined) => {
    return val !== null && val !== undefined ? val : '--';
  };

  // Funció auxiliar per traduir graus a punts cardinals
      ...
      <div className="flex items-center justify-between p-4 md:p-5 rounded-2xl bg-gradient-to-br from-[#151725] to-[#0d0f18] border border-white/5 hover:border-white/10 transition-colors shadow-lg group">
        <div className="flex flex-col gap-1">
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-[10px] text-slate-400 font-black uppercase tracking-widest">Dinàmica de Vent</span>
          </div>
          ...
          <div className="text-[11px] font-mono font-medium text-slate-500 mt-1">
            RATXES: <span className="text-indigo-300 font-bold tabular-nums">{formatValue(windGusts)}</span> km/h
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="flex flex-col p-4 rounded-2xl bg-[#151725] border border-white/5 hover:border-white/10 transition-colors group">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-[10px] text-slate-400 font-black uppercase tracking-widest">Humitat</span>
          </div>
        ...
        <div className="flex flex-col p-4 rounded-2xl bg-[#151725] border border-white/5 hover:border-white/10 transition-colors group">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-[10px] text-slate-400 font-black uppercase tracking-widest">Sensació</span>
          </div>
          <div className="flex items-baseline gap-1 mt-auto">
