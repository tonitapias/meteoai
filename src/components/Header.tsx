...
import { Search, MapPin, Loader2, X, Star, Trash2, LocateFixed } from 'lucide-react';
...
export default function Header({ onSearch, onLocate, loading }: HeaderProps) {
  const { favorites, removeFavorite } = usePreferences();
  ...
  useEffect(() => {
    const timer = setTimeout(async () => {
      ...
      if (trimmedQuery.length > 2) {
        ...
        try {
            const res = await fetch(`https://geocoding-api.open-meteo.com/v1/search?name=${encodeURIComponent(trimmedQuery)}&count=5&language=en&format=json`);
  ...
  const handleKeyDown = (e: React.KeyboardEvent) => {
    ...
    const currentList = isFavoritesMode ? favorites : results;
    ...
    if (e.key === 'ArrowDown') {
      ...
      } else if (!isFavoritesMode && results.length > 0) {
  ...
  return (
    <header className="w-full flex flex-col md:flex-row items-center gap-4 relative z-50 pointer-events-none md:pointer-events-auto">
        ...
        {/* BARRA DE CERCA CENTRADA 
            - En mòbil: w-full (flux normal)
            - En PC: absolute left-1/2 -translate-x-1/2 (centrat perfecte)
        */}
                        ...
                        {isFavoritesMode && favorites.length > 0 && (
                            <>
                                <div className="px-4 py-2 text-[10px] font-bold text-slate-500 uppercase tracking-widest bg-white/5 flex items-center gap-2">
                                    <Star className="w-3 h-3 text-amber-400" fill="currentColor" /> Favorits
                                </div>
        ...
        {/* BOTONS D'ACCIÓ (Sempre a la dreta) */}
        <div className="flex items-center gap-3 pointer-events-auto md:ml-auto z-10">
            <button 
                ...
                title="Utilitzar la meva ubicació"
            >
