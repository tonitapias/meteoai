...
import { FlagIcon } from './WeatherUI'; // Assumint que WeatherUI també es migrarà
...
export default function Header({ 
  ...
  const handleKeyDown = (e: React.KeyboardEvent) => {
    ...
    // Unifiquem tipus per la llista de navegació (LocationData o SuggestionItem tenen estructures similars pel que fa a l'UI)
    const list: any[] = isFavoritesView ? favorites : suggestions;
  ...
  const cycleLang = () => {
    const langs: Language[] = ['ca', 'es', 'en', 'fr'];
  ...
  return (
    <header className="relative z-50">
      ...
      {/* DISPOSICIÓ */}
      <div className="bg-slate-900/60 backdrop-blur-xl border border-white/10 rounded-2xl p-4 shadow-2xl flex flex-wrap md:flex-nowrap gap-4 items-center justify-between transition-all duration-300 relative z-50">
        
        {/* LOGO */}
        <div className="flex items-center gap-3 group cursor-default order-1">
        ...
        {/* CONTROLS DRETA */}
        <div className="flex items-center gap-3 w-auto order-2 md:order-3 ml-auto md:ml-0 justify-end relative z-50">
        ...
        {/* CERCA */}
        <div className="w-full md:w-auto md:flex-1 md:max-w-xl relative group/search order-3 md:order-2 z-50">
               ...
               {query.length <= 2 && favorites.length > 0 && (
                   <div className="p-2">
                       ...
                       {favorites.map((fav, idx) => (
                           <div 
                             key={fav.name} 
                             onClick={() => { handleSelectSuggestion(fav); }} 
