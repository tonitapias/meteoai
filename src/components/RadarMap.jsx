...
// Correcció per icones de Leaflet en React
...
// Component per moure el mapa si canvien les coordenades
...
export default function RadarMap({ lat, lon }) {
  ...
  // 1. Carregar configuració de RainViewer
  useEffect(() => {
    const fetchRadarConfig = async () => {
      try {
        ...
        // Unim dades passades i predicció (nowcast) i ordenem
        const allFrames = [
           ...(data.radar?.past || []), 
           ...(data.radar?.nowcast || [])
        ...
        setCurrentIndex(allFrames.length - 6); // Iniciem una mica abans del final
        setLoading(false);
        setIsPlaying(true); 
      } catch (e) {
  ...
  return (
    <div className="relative w-full h-full rounded-2xl overflow-hidden border border-white/10 shadow-2xl bg-slate-900">
      ...
      >
        ...
        {/* Marcador Ubicació Usuari (punt blau) */}
      ...
      <div className="absolute bottom-6 left-6 right-6 z-[1000] bg-slate-900/90 backdrop-blur-md p-4 rounded-xl border border-white/10 flex items-center gap-4 shadow-xl">
        ...
        <div className="flex-1">
            ...
            {/* Barra de progrés personalitzada */}
            <input 
                type="range" 
                min="0" 
