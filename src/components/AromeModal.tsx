...
export default function AromeModal({ lat, lon, onClose }: AromeModalProps) {
  ...
  const maxCape = aromeData ? Math.max(...(aromeData.hourly.cape || [0])) : 0;
  const totalRain = aromeData ? aromeData.hourly.precipitation.reduce((a: number, b: number) => a + b, 0) : 0;
                ...
                <div>
                    ...
                    <div className="grid grid-cols-3 gap-2 p-4 bg-slate-800/30 border-b border-white/5">
                         <div className="text-center">
                            ...
                            <div className="text-xl font-bold text-blue-200">{totalRain.toFixed(1)}<span className="text-xs font-normal text-slate-500 ml-0.5">mm</span></div>
                         </div>
                         <div className="text-center border-l border-white/5">
                            <div className="text-[10px] uppercase text-slate-500 font-bold mb-1">Ràfega Màx</div>
                            <div className={`text-xl font-bold ${maxGust > 50 ? 'text-teal-300' : 'text-slate-200'}`}>{Math.round(maxGust)}<span className="text-xs font-normal text-slate-500 ml-0.5">km/h</span></div>
                         </div>
                         <div className="text-center border-l border-white/5">
                            <div className="text-[10px] uppercase text-slate-500 font-bold mb-1">Energia</div>
                            <div className={`text-xl font-bold ${maxCape > 800 ? 'text-amber-400' : 'text-slate-200'}`}>{Math.round(maxCape)}<span className="text-xs font-normal text-slate-500 ml-0.5">J/kg</span></div>
                         </div>
                    </div>
                    ...
                    <div className="divide-y divide-white/5">
                        {hourlyRows.map((row, index) => {
                            ...
                            // Lògica per detectar NEU (Temperatura baixa)
                            const isSnow = row.temp < 1.5; 
                                ...
                                <div key={row.time} className="relative group">
                                    ...
                                    <div className={`flex items-center justify-between p-4 transition-colors ${isRaining ? 'bg-blue-900/10' : 'hover:bg-white/5'}`}>
                                        ...
                                        <div className="flex-1 flex justify-center items-center gap-6">
                                            ...
                                            {/* ZONA DE PRECIPITACIÓ MILLORADA */}
                                            <div className="w-20 flex flex-col items-center">
                                                {row.precip > 0 ? (
                                                    <>
                                                        <div className={`flex items-center gap-1 font-bold ${isSnow ? 'text-white' : 'text-blue-300'}`}>
                                                            {isSnow ? <Snowflake className="w-3 h-3" /> : <Droplets className="w-3 h-3" />}
                                                            {row.precip >= 10 ? Math.round(row.precip) : row.precip.toFixed(1)}
                                                            <span className="text-[10px] font-normal opacity-70 ml-0.5">{isSnow ? 'cm' : 'mm'}</span>
                                                        </div>
                                                        <div className="h-1 w-full bg-slate-700 rounded-full mt-1 overflow-hidden">
                                                            <div className={`h-full ${isSnow ? 'bg-white' : 'bg-blue-500'}`} style={{width: `${Math.min(row.precip * 10, 100)}%`}}></div>
                                                        </div>
                                                    </>
                                                ) : (
