...
export default function WindMap({ lat, lon }: WindMapProps) {
  ...
  const currentLang = i18n.language?.split('-')[0] || 'ca'; // Agafem 'ca', 'es', etc.
  ...
  const windyUrl = isValidCoords 
    ? `https://embed.windy.com/embed.html?type=map&location=coordinates&metricRain=mm&metricTemp=default&metricWind=km%2Fh&zoom=9&overlay=wind&product=ecmwf&level=surface&lat=${lat}&lon=${lon}&lang=${currentLang}`
    : '';
        ...
        <div className="w-full h-full min-h-0 bg-[#020308] relative overflow-hidden flex items-center justify-center">
            <div className="text-center z-10 flex flex-col items-center">
                ...
                <span className="text-slate-400 text-xs font-mono font-bold uppercase tracking-widest">
                  {t('errCoords', 'Error de Coordenades')}
                </span>
            </div>
        </div>
      );
    ...
    <div className="w-full h-full min-h-0 bg-[#020308] relative overflow-hidden flex items-center justify-center">
      ...
      {hasError && (
        <div className="absolute inset-0 z-30 flex flex-col items-center justify-center bg-gradient-to-br from-[#0f111a]/95 to-black/90 backdrop-blur-md">
             ...
             <span className="text-white font-black tracking-widest uppercase mb-1 drop-shadow-md z-10">
               {t('errWindDown', 'Connexió ECMWF Caiguda')}
             </span>
             <span className="text-xs text-slate-400 font-mono font-bold mb-6 z-10">
               {t('errWindDesc', "Temps d'espera esgotat per Windy")}
             </span>
