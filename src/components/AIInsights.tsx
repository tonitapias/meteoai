...
const LOCAL_UI_TEXTS: Record<string, LocalUIText> = {
    ca: {
        ...
        hazards: {
            ...
            AIR_QUALITY: "QUALITAT DE L'AIRE DEFICIENT O POLS EN SUSPENSIÓ"
        }
    },
    es: {
        ...
        hazards: {
            ...
            AIR_QUALITY: 'CALIDAD DEL AIRE DEFICIENTE O POLVO EN SUSPENSIÓN'
        }
    },
    fr: {
        stable: 'TEMPS CALME',
        ...
        hazards: {
            ...
            AIR_QUALITY: "MAUVAISE QUALITÉ DE L'AIR OU POUSSIÈRE EN SUSPENSION"
        }
    },
    en: {
        stable: 'CALM WEATHER',
        ...
        hazards: {
            ...
            AIR_QUALITY: 'POOR AIR QUALITY OR AIRBORNE DUST'
        }
    }
};

// --- SUB-COMPONENTS D'ESTIL CLARS I ENTENEDORS (SPATIAL UI SENSE SOBRECÀRREGA GPU) ---

/**
 * SEMÀFOR DE RISC ENTENEDOR (Fons sòlids per a millor contrast outdoor)
 */
const TacticalRiskBadge = ({ analysis, ui }: { analysis: TacticalAnalysisResult; ui: LocalUIText }) => {
    ...
    const styles: Record<TacticalRiskLevel, { badge: string; dot: string; label: string }> = {
        GREEN: {
            badge: 'text-emerald-300 border-emerald-500/40 bg-emerald-950/80 shadow-[inset_0_1px_0_rgba(255,255,255,0.1),0_0_10px_rgba(16,185,129,0.1)]',
            dot: 'bg-emerald-400 shadow-[0_0_5px_rgba(16,185,129,0.8)] animate-pulse',
            label: ui.stable
        },
        AMBER: {
            badge: 'text-amber-300 border-amber-500/40 bg-amber-950/80 shadow-[inset_0_1px_0_rgba(255,255,255,0.1),0_0_10px_rgba(245,158,11,0.1)]',
            dot: 'bg-amber-400 shadow-[0_0_5px_rgba(245,158,11,0.8)] animate-pulse',
            label: ui.warning
        },
        RED: {
            badge: 'text-rose-200 border-rose-500/50 bg-rose-950/80 shadow-[inset_0_1px_0_rgba(255,255,255,0.2),0_0_15px_rgba(244,63,94,0.3)]',
...
/**
 * BÀNER DE PERILL ATMOSFÈRIC CLAR
 */
const TacticalHazardBanner = ({ hazardType, riskLevel, ui }: { hazardType?: TacticalHazardType; riskLevel?: TacticalRiskLevel; ui: LocalUIText }) => {
    ...
    // Fons enfosquit per contrast sota el sol, sense blurs innecessaris
    return (
        <div className={`relative flex items-center gap-3.5 p-3.5 sm:p-4 rounded-xl border overflow-hidden shadow-lg ${
            isSevere 
            ...
            <div className="flex-1 min-w-0 relative z-10">
                <p className="text-sm sm:text-base leading-tight font-sans">
                    <span className="font-mono text-[10px] sm:text-xs uppercase opacity-80 tracking-widest mr-1">{ui.reason}:</span>{' '}
                    <span className={`font-extrabold tracking-wide ${isSevere ? 'text-rose-200' : 'text-amber-200'}`}>
                        {hazardText}
                    </span>
                </p>
            </div>
...
const InsightAlert = ({ alert }: { alert: AlertItem }) => {
    ...
    return (
        <div className={`relative flex items-center gap-3.5 p-3.5 sm:p-4 rounded-xl border overflow-hidden shadow-lg ${
            ...
            <div className="flex-1 min-w-0 relative z-10">
                <p className="text-sm sm:text-base leading-tight font-sans">
                    {alert.type && <span className="font-mono text-[10px] sm:text-xs uppercase opacity-80 tracking-widest mr-1">{alert.type}:</span>}{' '}
                    <span className={`font-extrabold tracking-wide ${isHigh ? 'text-rose-200' : 'text-amber-200'}`}>
                        {alert.msg}
                    </span>
                </p>
            </div>
...
/**
 * PÍNDOLA D'INFORMACIÓ RÀPIDA (Sense Blur Interior per rendiment)
 */
const InsightTip = ({ tip, index }: { tip: TacticalTip | string; index: number }) => {
    ...
    let Icon = Activity;
    let badgeStyle = "bg-slate-900/80 hover:bg-slate-800 text-cyan-100 border-cyan-500/40 ring-1 ring-inset ring-cyan-500/10 shadow-md";
    let iconColor = "text-cyan-400 drop-shadow-[0_0_4px_rgba(6,182,212,0.8)]";

    switch (category) {
        case 'WIND':
            Icon = Wind;
            badgeStyle = "bg-slate-900/80 hover:bg-slate-800 text-cyan-100 border-cyan-500/40 ring-1 ring-inset ring-cyan-500/10 shadow-md";
            iconColor = "text-cyan-400 drop-shadow-[0_0_4px_rgba(6,182,212,0.8)]";
            break;
        case 'THERMAL':
            Icon = Thermometer;
            badgeStyle = "bg-amber-950/80 hover:bg-amber-900/90 text-amber-100 border-amber-500/40 ring-1 ring-inset ring-amber-500/10 shadow-md";
            iconColor = "text-amber-400 drop-shadow-[0_0_4px_rgba(245,158,11,0.8)]";
            break;
        case 'SKY':
            Icon = Layers;
            badgeStyle = "bg-indigo-950/80 hover:bg-indigo-900/90 text-indigo-100 border-indigo-500/40 ring-1 ring-inset ring-indigo-500/10 shadow-md";
            iconColor = "text-indigo-400 drop-shadow-[0_0_4px_rgba(99,102,241,0.8)]";
            break;
        case 'HAZARD':
            Icon = ShieldAlert;
            badgeStyle = "bg-rose-950/80 hover:bg-rose-900/90 text-rose-100 border-rose-500/50 ring-1 ring-inset ring-rose-500/10 shadow-md";
            iconColor = "text-rose-400 animate-pulse drop-shadow-[0_0_6px_rgba(244,63,94,0.8)]";
            break;
        default:
            Icon = Activity;
            break;
    ...
    return (
        <div 
            className={`text-xs sm:text-sm px-3.5 py-2 rounded-xl border flex items-start sm:items-center gap-2.5 transition-all animate-in zoom-in-95 duration-300 fill-mode-backwards shrink-0 max-w-full break-words ${badgeStyle}`} 
...
export default function AIInsights({ analysis, lang, isLoading = false, hasError = false, onRetry }: AIInsightsProps) { 
    ...
    // Només el contenidor pare utilitza backdrop-blur per salvaguardar la GPU
    return (
        <div className={`flex flex-col w-full h-full min-h-[200px] sm:min-h-[220px] rounded-2xl sm:rounded-3xl backdrop-blur-2xl shadow-[0_8px_32px_rgba(0,0,0,0.5)] relative overflow-hidden transition-all duration-700 transform-gpu translate-z-0 ring-1 ring-inset ${
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar p-4 sm:p-5 md:p-6 pt-3.5 sm:pt-4 relative z-10 flex flex-col justify-between gap-5">
                <div className="animate-in fade-in duration-700 space-y-4">
                    ...
                    {/* Risc Zero: Protegit amb encadenament opcional (?.) */}
