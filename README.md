...
* **Gràfic de tendència de 7 dies amb la incertesa a la vista:** cada dia mostra la màxima i la mínima amb el seu rang probable (bigotis, vegeu més avall) i tres punts de fiabilitat segons l'acord entre models (sobre la màxima i la mínima), perquè un canvi d'1-2° entre dos dies no es llegeixi com una tendència quan la incertesa és més gran que això. Una tira indica de quin model surt cada dia (el regional als primers dies, el global després: un salt entre tots dos no és un canvi de temps), una frase resum diu si s'escalfa, es refreda o és variable —i avisa quan el canvi és menor que el desacord entre models— i cada dia obre el seu detall.
...
* **Rang probable de la temperatura (80 %), calibrat contra observacions:** el gràfic de tendència i el detall de dia dibuixaven el rang entre models (el mínim i el màxim d'ECMWF, GFS, ICON i best_match), que diu si els models coincideixen però no és un marge d'error: contrastat amb 76.755 dies de 30 aeroports europeus (un any de previsions d'1 a 7 dies contra els METAR), només contenia la temperatura real el 48 % de les vegades, i el 37 % al dia 7. Ara cada dia porta el rang dins del qual queden la màxima i la mínima reals 8 de cada 10 vegades, construït amb els errors reals de la xifra que mostra l'app segons el dia vista i l'acord entre models: el conté el 80 % de les vegades (validat deixant cada aeroport fora del calibratge; per aeroport, del 68 al 91 %). S'eixampla amb el dia vista i amb el desacord entre models (la màxima fa uns 4 °C d'ample quan els models coincideixen i uns 10 °C quan discrepen molt) i no és simètric: la mínima real acostuma a quedar una mica per sota de la mostrada. Límit: està calibrat amb aeroports, i en relleu complex l'error real pot ser més gran.
* **Correcció d'inversió tèrmica mesurada i reduïda a la meitat:** a les nits serenes i en calma de novembre a març l'app refreda la temperatura del model, que no veu l'aire fred que s'acumula a valls i conques. Contrastada amb l'hivern 2025-26 a 30 aeroports, la correcció de fins a 3,5 °C empitjorava la temperatura de les hores on s'aplicava (error mitjà d'1,38 a 1,55 °C; només millorava 9 dels 29 aeroports on salta): el model ja hi té un biaix càlid de només +0,6 °C. Amb la meitat (fins a 1,75 °C) l'error baixa a 1,33 °C i la mínima diària també encerta més. La sensació tèrmica porta la mateixa correcció que la temperatura, a la capçalera i al que rep la IA.
* **L'"ara", la capçalera i les hores diuen el mateix:** la màxima i la mínima de la capçalera surten de les mateixes hores que el detall d'«Avui» i la llista (abans eren el diari del model global en brut: a Girona deia 28° de màxima amb 31° a les hores); la intensitat de la pluja d'"ara" es compta en mm/h com a les hores (les dades de 15 minuts la feien semblar 4 vegades més feble: 2 mm/h eren "pluja feble" a dalt i "moderada" a la taula), i el punt de rosada i la direcció del vent surten del mateix model que la temperatura, la humitat i la velocitat del vent.
* **Confiança de l'anàlisi de la IA sobre les mateixes 6 hores de què parla:** la insígnia de l'anàlisi +6h (consens, divergència o incertesa alta) reutilitzava la fiabilitat del dia sencer, que de nit jutjava hores ja passades. Mesurat amb 262.617 finestres de 6 hores de 30 aeroports europeus (un any de previsions de curt termini contra els METAR), no avisava de res: quan deia «incertesa alta», la previsió fallava (més de 3 °C a la temperatura, o si plourà o no) tan sovint com quan deia «consens», un 18 %. Ara compara, hora a hora, la sèrie que veus (el model regional o el global) amb ECMWF, GFS, ICON i AIFS: quant discrepen entre ells i quant se separa del que diuen els globals, i quants globals contradiuen si plourà. La previsió falla un 10 % de les vegades quan la insígnia diu «consens», un 26 % amb un dubte moderat i un 56 % amb un dubte fort. A més, diu què és incert, perquè les dues causes es comporten diferent (quan dubta la temperatura, la pluja gairebé no falla, i a l'inrevés): «Temperatura ±3°» (o ±4°, ±5°) en ambre quan els models només discrepen en la temperatura —el marge és on queda la real en 8 de cada 10 casos—, «Pluja dubtosa» o, en vermell, «Pluja incerta» quan discrepen en si plourà, i els textos generals quan dubten totes dues coses; en passar-hi per sobre, una frase ho explica. Si falten dades o no hi ha prou models per comparar, la insígnia no surt, en lloc de posar un consens o una incertesa inventats. Límits: mesura acord, no exactitud; en nits d'inversió on els models no s'hi posen d'acord (Girona, per exemple) surt un marge de temperatura ampli, en ambre, encara que el temps sigui tranquil.
...
* **Escala d'inestabilitat completa:** entre 100 i 1.200 J/kg l'aire ja no surt com a «Estable» sinó com a inestabilitat «Feble» —la mateixa verificació de la regla de tempesta troba trons observats amb 800 J/kg—; per sota de 100 J/kg sí que és estable. Les etiquetes del giny surten en l'idioma de l'usuari (abans sempre en català).
...
* **Cisallament del vent (0–6 km):** al pic de CAPE de la finestra, la diferència entre el vent a 10 m i a 500 hPa diu com s'organitzarien les tempestes si n'hi ha: aïllades i poc organitzades (< 36 km/h), organitzades o multicèl·lules (36–72 km/h) o amb supercèl·lules possibles (≥ 72 km/h), amb els llindars de la literatura operativa. Els dos vents surten sempre del mateix model global (ECMWF, o si no GFS o ICON), mai barrejats. És informació: no toca la icona ni el nivell de risc, i no s'ha pogut verificar localment perquè els METAR de 30 aeroports només registren 22 hores de calamarsa en un any.
* **Estadístiques ràpides:** CAPE màxim a 24h i 48h, total d'hores en risc i cisallament actual (el nivell de congelació ja surt a la capçalera del modal).
* Reutilitza dades horàries que l'app ja demanava a Open-Meteo, més el vent a 500 hPa dins de la mateixa petició — cap crida de xarxa nova ni dependència externa.
...
* **Finestra de protecció:** calcula i mostra l'hora d'inici i final en què cal protegir-se (llindar oficial OMS, índex ≥ 3), amb un estat "actiu ara mateix" quan pertoca, o confirma honestament que avui no cal protecció.
* **Bandes oficials OMS:** reutilitza exactament el mateix sistema de 5 categories (Baix/Moderat/Alt/Molt Alt/Extrem) que ja fa servir el Cicle Solar per al UV màxim diari — cap llindar nou inventat. Com demana l'OMS, la categoria i la protecció es decideixen sobre l'índex arrodonit a enter (el número tal com es mostra, amb un decimal): abans un 5,8 sortia «Moderat» en lloc d'«Alt».
...
* **Evolució de 48h** del punt de rosada (`dew_point_2m`), *scrubbable*, amb la humitat relativa de cada hora com a dada secundària — totes dues del mateix model que la temperatura (el regional on n'hi ha; abans el punt de rosada era sempre del global i a Barcelona podia diferir fins a 9,8 °C).
...
* **Cota actual i nivell de congelació:** la cota de neu (altitud on la pluja passa a neu, ≈300m per sota de la isoterma 0°C) com a valor principal, amb el nivell de congelació mostrat a part com a dada de context. El marge de 300 m està verificat contra 13.438 hores de precipitació observada a 30 aeroports: és el millor marge fix (200 o 400 m encerten menys) i fer-lo dependre de la humitat de superfície ho empitjora, perquè prop de la cota la humitat de terra gairebé no separa la neu de la pluja.
* **Centímetres de neu coherents amb la icona:** la neu acumulada surt de la mateixa pluja i del mateix motor que decideix la icona de cada hora (0,7 cm per mm a les hores de neu, la meitat a les d'aiguaneu). Abans eren els centímetres del model global, amb la seva pròpia partició pluja/neu: en un hivern de 30 aeroports, 108 de 234 dies amb neu en joc contradeien la icona (icona de neu amb la xifra en mm, o centímetres amb icona de pluja). Ara no en queda cap amb centímetres i icona de pluja, i l'encert per saber si un dia neva és el mateix.
